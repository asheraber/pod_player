...
class _PodVideoQualityController extends _PodVideoController {
  ...
  Future<void> getQualityUrlsFromVimeoId(
    ...
    try {
      ...
      // Handle 410 Gone errors by retrying once
      if (e is Vimeo410Exception && retryCount < 1) {
        podLog('===== VIMEO 410 RETRY =====');
        podLog('Video ID: $videoId');
        podLog('Attempt: ${retryCount + 1}/1');
        podLog('Refreshing progressive links...');
        // Wait a short delay before retry
        await Future<void>.delayed(const Duration(milliseconds: 500));
        // Retry once to get fresh progressive links
        return getQualityUrlsFromVimeoId(
          videoId,
          hash: hash,
  ...
  Future<void> getQualityUrlsFromVimeoPrivateId(
    ...
    try {
      ...
      // Handle 410 Gone errors by retrying once
      if (e is Vimeo410Exception && retryCount < 1) {
        podLog('===== VIMEO PRIVATE API 410 RETRY =====');
        podLog('Video ID: $videoId');
        podLog('Attempt: ${retryCount + 1}/1');
        podLog('Refreshing progressive links...');
        // Wait a short delay before retry
        await Future<void>.delayed(const Duration(milliseconds: 500));
        // Retry once to get fresh progressive links
        return getQualityUrlsFromVimeoPrivateId(
          videoId,
          httpHeader,
          retryCount: retryCount + 1,
        );
